...
function seedViaRpc() {
  ...
  return new Promise((resolve, reject) => {
    ...
    function sendConnect() {
      ...
      console.log('[rpc-seed] Sending connect handshake...');
    ...
    ws.addEventListener('message', (event) => {
      ...
      try { msg = JSON.parse(raw); } catch (e) {
        console.warn('[rpc-seed] Non-JSON message:', raw.slice(0, 120));
        return;
      }

      // Debug: log message types during handshake
      if (!handshakeSent) {
        console.log(`[rpc-seed] recv type=${msg.type} event=${msg.event || 'n/a'}`);
      }
      ...
      if (!handshakeSent) {
        console.log(`[rpc-seed] Unrecognized first msg type=${msg.type}, sending connect anyway`);
        sendConnect();
        return;
      }
