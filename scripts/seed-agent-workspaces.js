...
|---|---------|-------------|
| 1 | Rules & Behavior | Act don't ask, token budget, file protection |
| 2 | Your Permissions | What tools YOU can use, writable paths, restrictions |
| 3 | Core Tools & Syntax | Tool table, exact call formats, file rules |
...
| 11 | Mandatory Checklist | Activity log, staging, confirm — EVERY task |
...
## 10. RECOVERY — When You're Stuck (NEVER message the owner about this)

**Step 1: Read your workspace files.**
\`read(path: "TOOLS.md")\` — Check the TABLE OF CONTENTS to find the right section
...
## 11. MANDATORY CHECKLIST (EVERY task, NO exceptions)
...
const TOOL_PERMISSIONS = {
  // Full access agents (gateway DENIED for all — agents previously crashed OpenClaw by writing invalid config keys)
  'codecraft': 'You have FULL access to ALL tools EXCEPT gateway. No approval needed. No sandbox.\n- **All tools enabled:** read, write, edit, exec, sessions_send, sessions_list, sessions_history, memory_search, web_fetch, cron, agents_list, browser\n- **Shell access:** exec runs on the OpenClaw container (has wget, node — NO curl)\n- **gateway DENIED:** System config is owner-managed only. Do NOT use config.set, config.patch, or any gateway write operation.',
  'builder': 'You have FULL access to ALL tools EXCEPT gateway. No approval needed. No sandbox.\n- **All tools enabled:** read, write, edit, exec, sessions_send, sessions_list, sessions_history, memory_search, web_fetch, cron, agents_list, browser\n- **Shell access:** exec runs on the OpenClaw container (has wget, node — NO curl)\n- **gateway DENIED:** System config is owner-managed only. Do NOT use config.set, config.patch, or any gateway write operation.',
  'sentinel': 'You have FULL access to ALL tools EXCEPT gateway. No approval needed. No sandbox.\n- **All tools enabled:** read, write, edit, exec, sessions_send, sessions_list, sessions_history, memory_search, web_fetch, cron, agents_list, browser\n- **Shell access:** exec runs on the OpenClaw container (has wget, node — NO curl)\n- **gateway DENIED:** System config is owner-managed only. Do NOT use config.set, config.patch, or any gateway write operation.',
  // Restricted agents
  'lead': 'You have access to ALL tools EXCEPT browser and gateway. No approval needed. No sandbox.\n- **Tools enabled:** read, write, edit, exec, sessions_send, sessions_list, sessions_history, memory_search, web_fetch, cron, agents_list\n- **Shell access:** exec runs on the OpenClaw container (has wget, node — NO curl)\n- **Browser denied:** Delegate browser tasks to CodeCraft or Builder.\n- **gateway DENIED:** System config is owner-managed only.',
  'ops-lead': 'You have access to ALL tools EXCEPT browser and gateway. No approval needed. No sandbox.\n- **Tools enabled:** read, write, edit, exec, sessions_send, sessions_list, sessions_history, memory_search, web_fetch, cron, agents_list\n- **Shell access:** exec runs on the OpenClaw container (has wget, node — NO curl)\n- **Browser denied:** Delegate browser tasks to Builder.\n- **gateway DENIED:** System config is owner-managed only.',
  'scout': 'You have access to ALL tools EXCEPT exec and gateway. No approval needed. No sandbox.\n- **Tools enabled:** read, write, edit, sessions_send, sessions_list, sessions_history, memory_search, web_fetch, cron, agents_list, browser\n- **exec denied:** Use web_fetch for URLs. For web search or scraping, ask CodeCraft or Builder to run SearXNG/Scrapling wget commands.\n- **Workaround for search:** Message CodeCraft: "Search SearXNG for <query> and send me the results"\n- **gateway DENIED:** System config is owner-managed only.',
  'scribe': 'You have access to file and session tools. No approval needed. No sandbox.\n- **Tools enabled:** read, write, edit, sessions_send, sessions_list, sessions_history, memory_search, web_fetch, cron, agents_list\n- **exec denied:** You write documentation, not shell commands. If you need data, ask Scout or CodeCraft.\n- **browser denied:** Delegate browsing tasks to CodeCraft.\n- **gateway DENIED:** System config is owner-managed only.',
...
First check \`cron(action: "list")\` — if you already have a heartbeat cron (*/2 or */4 schedule), SKIP this. Remove duplicates if found.
Only if you have ZERO heartbeat crons, create one:
\`cron(action: "add", schedule: {type: "cron", expression: "0 */4 * * *"}, payload: {kind: "agentTurn", message: "HEARTBEAT: Execute HEARTBEAT.md checklist now.", session: "isolated"}, target: {agentId: "<your-id>"})\`
